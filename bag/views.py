"""
Bag views to display bag template content and redirects
"""
    ...
    if product_size:
        if item_id in list(bag.keys()):
            if product_size in bag[item_id]['items_by_size'].keys():
                ...
                messages.success(
                         request,
                         f'Updated size {product_size.upper()} {product.name}\
                         quantity\
                         to {bag[item_id]["items_by_size"][product_size]}')

            else:
                bag[item_id]['items_by_size'][product_size] = quantity
                messages.success(
                         request,
                         f'Added size {product_size.upper()} {product.name}\
        ...
        if item_id in list(bag.keys()):
            ...
            messages.success(
                     request,
                     f'Added {product.name} to your shopping bag successfully')
...
def adjust_bag(request, item_id):
    ...
    if product_size:
        if quantity > 0:
            ...
            messages.success(
                     request,
                     f'Removed size {product_size.upper()} {product.name}'
                     'from your shopping bag successfully')
    else:
        if quantity > 0:
            bag[item_id] = quantity
            messages.success(
